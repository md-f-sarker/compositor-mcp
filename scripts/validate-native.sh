...
FLAGS="-swift-version 5 -enable-upcoming-feature NonisolatedNonsendingByDefault -enable-upcoming-feature InferIsolatedConformances -sdk $SDK -target $TARGET"
...
cd "$COMPOSITOR"
swiftc -typecheck $FLAGS -I "$WORK/sparkle-stub" $(find Compositor -name "*.swift") 2>&1 | tee "$WORK/typecheck.log" | grep "error:" || true
